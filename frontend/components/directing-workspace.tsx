...
// Display one concise structured list without empty placeholders.
function GuidanceSection({ label, items }: { label: string; items: string[] }) {
    if (items.length === 0) return null;
    return <section className="rounded-xl border border-slate-200 p-5"><h3 className="font-bold text-slate-950">{label}</h3><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600">{items.map((item) => <li key={item} className="flex gap-2"><span className="mt-2 size-1.5 shrink-0 rounded-full bg-violet-500" />{item}</li>)}</ul></section>;
}
